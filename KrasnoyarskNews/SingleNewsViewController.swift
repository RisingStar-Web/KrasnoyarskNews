...
//            cell.timeLabel.text = news.time
            ...
            if let backgroundImage = UIImage(contentsOfFile: NewsParser.getDocumentsDirectory().appendingPathComponent("\(news.title!)"+"\(news.date!)"+".jpg").path) {
                ...
                DispatchQueue.global(qos: .userInitiated).async {
                    ...
                    NewsParser.downloadCityImage(path: self.news.picture, name: "\(self.news.title!)"+"\(self.news.date!)"+".jpg", completion:{ (success: Bool) -> Void in
                        if success {
