...
class NewsParser: NSObject, XMLParserDelegate {
    ...
    class func downloadNews(completion: @escaping ((_ success: Bool) -> Void)) {
        var newsArray: [News] = []
        let url = URL(string: "https://raw.githubusercontent.com/ap-consulting/serverxml_/master/news(m).xml")!
...
//                let date = dateFormatter.date(from: news.date.text!)
//                let item = News.init(date: date!, time: (news.time.text ?? " "), title: (news.title.text ?? " "), shortDescription: (news.shortDescription.text ?? " "), fullDescription: (news.fullDescription.text ?? " "), croppedPicture: (news.croppedPicture.text ?? " "), picture: (news.picture.text ?? " "))
//                newsArray.append(item)
                
//                let item = News.init(date: (news.date.text ?? " "), time: (news.time.text ?? " "), title: (news.title.text ?? " "), shortDescription: (news.shortDescription.text ?? " "), fullDescription: (news.fullDescription.text ?? " "), croppedPicture: (news.croppedPicture.text ?? " "), picture: (news.picture.text ?? " "))
//                newsArray.append(item)
//            }
//            DispatchQueue.main.async {
//                completion(false, newsArray)
//            }
        }
        task.resume()
    }
